...
{
    public class JokeApiClient : IWidgetApiClient
    {
        private readonly HttpClient _httpClient;
        private readonly string _apiUrl;
        public string Name { get; set; }

        public JokeApiClient(HttpClient httpClient, Widget widget)
        {
            if (widget == null)
        ...
        public async Task<KeyValuePair<string, object>> ObtenerDatosAsync(string name)
        {
            ...
            {
                throw new Exception($"Error al realizar la solicitud a la API de Chistes: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
                throw new Exception($"Error inesperado al obtener datos de Chistes: {ex.Message}", ex);
            }
        }
    }
}
