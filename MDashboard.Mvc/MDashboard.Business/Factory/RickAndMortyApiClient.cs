...
{
    public class RickAndMortyApiClient : IWidgetApiClient
    {
        private readonly HttpClient _httpClient;
        private readonly string _apiUrl;
        public string Name { get; set; }

        public RickAndMortyApiClient(HttpClient httpClient, Widget widget)
        {
            if (widget == null)
        ...
        public async Task<KeyValuePair<string, object>> ObtenerDatosAsync(string name)
        {
            try
            {
                ...
                var apiResponse = JsonConvert.DeserializeObject<RickAndMortyApiResponse>(content);

                return new KeyValuePair<string, object>(name, apiResponse);
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Error al realizar la solicitud a la API de Rick and Morty: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
                throw new Exception($"Error inesperado al obtener datos de Rick and Morty: {ex.Message}", ex);
            }
        }
    }
}
